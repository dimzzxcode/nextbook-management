...
import { updateAuthorAction } from "@/modules/authors/actions/author.actions";

export default function EditAuthorForm({ author }: { author: { id: number; name: string; biography: string | null } }) {
  ...
  const handle = (e: React.FormEvent) => {
    ...
    start(async () => {
      const res = await updateAuthorAction({ id: author.id, name, biography: biography || undefined });
    ...
    <form onSubmit={handle} noValidate>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} style={{ width: "100%", height: 40, padding: "0 12px", border: `1px solid ${fieldErrors.name ? "#d93636" : "var(--border)"}`, borderRadius: 6 }} />
        ...
        <div style={{ marginBottom: 0 }}>
          ...
          <textarea value={biography} onChange={(e) => setBiography(e.target.value)} rows={3} style={{ width: "100%", padding: 12, border: "1px solid var(--border)", borderRadius: 8, fontSize: 13, background: "var(--white)" }} />
      ...
      <div style={{ display: "flex", gap: 10, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
        ...
        <button type="submit" className="btn btn-primary" style={{ flex: 1 }} disabled={pending}>{pending ? "Menyimpan..." : "Update Penulis"}</button>
      </div>
    </form>
  );
}
