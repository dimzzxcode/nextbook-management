...
import { createCategoryAction } from "@/modules/categories/actions/category.actions";
...
export default function CreateCategoryForm() {
  ...
  const handle = (e: React.FormEvent) => {
    ...
    setServerError(null);
    setFieldErrors({});
    if (!name.trim()) { setFieldErrors({ name: "Nama kategori wajib diisi." }); return; }
    start(async () => {
      const res = await createCategoryAction({ name, description: description || undefined });
    ...
    <form onSubmit={handle} noValidate>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ marginBottom: 12 }}>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Nama Kategori <span style={{ color: "#d93636" }}>*</span></label>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Fiksi Ilmiah" style={{ width: "100%", height: 40, padding: "0 12px", border: `1px solid ${fieldErrors.name ? "#d93636" : "var(--border)"}`, borderRadius: 6, fontSize: 13 }} />
          {fieldErrors.name && <div style={{ fontSize: 12, color: "#d93636", marginTop: 4 }}>{fieldErrors.name}</div>}
          <div style={{ fontSize: 11.5, color: "var(--text-light)", marginTop: 4 }}>Nama harus unik, maks 100 karakter.</div>
        </div>
        <div style={{ marginBottom: 0 }}>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Deskripsi</label>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Jelaskan kategori ini, misal untuk buku sains populer..." style={{ width: "100%", padding: 12, border: "1px solid var(--border)", borderRadius: 8, fontSize: 13, fontFamily: "inherit", background: "var(--white)" }} />
      ...
      <div style={{ display: "flex", gap: 10, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
        ...
        <button type="submit" className="btn btn-primary" style={{ flex: 1 }} disabled={pending}>{pending ? "Menyimpan..." : "Simpan Kategori"}</button>
      </div>
    </form>
  );
}
